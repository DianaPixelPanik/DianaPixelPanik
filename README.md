## Hi there 👋

<!--
**DianaPixelPanik/DianaPixelPanik** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

# Hi, I'm Diana 👋

**Designer × Developer × Founder of [PixelPanik](https://github.com/DianaPixelPanik)**  
Based in Tallinn, Estonia 🇪🇪

---

## 🟢 Open to projects — let's work together

I'm actively looking for freelance and contract work.  
If you need a designer who can also build — I'm your person.

📩 **[dianaart1997@gmail.com](mailto:dianaart1997@gmail.com)**

---

## What I do

**UI/UX Design** — user research, wireframes, prototypes, Figma. I design interfaces that feel intuitive and look sharp.

**Frontend Development** — React, Next.js, TypeScript, Tailwind CSS. I don't just hand off designs — I build them.

**Product Design** — from zero to shipped. I help founders and teams turn ideas into real products: strategy, design system, dev-ready specs.

**AI Integration** — building products with AI at the core. Prompt engineering, LLM-powered interfaces, Claude / OpenAI APIs.

---

## Stack

![Figma](https://img.shields.io/badge/Figma-000?style=flat&logo=figma&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000?style=flat&logo=next.js)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)
![Tailwind](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white)

---

## Currently

- 🔨 Redesigning [eluliin.ee](https://eluliin.ee) — emotional support NGO in Estonia
- 🚀 Building products under **PixelPanik**
- 👀 Looking for interesting **design + dev projects**

---

*Available for freelance · contract · long-term collaboration*Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
